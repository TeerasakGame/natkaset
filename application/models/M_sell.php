<?php
	class M_sell extends CI_Model {
	    ...
	    {
	    	$sql = "SELECT * FROM nat_sell JOIN nat_member ON nat_member.mem_id = nat_sell.mem_id WHERE nat_sell.sel_status = 1 AND DATEDIFF(NOW(),sel_time_create)<=30 ORDER BY nat_sell.sel_time_create DESC";
	    	$query = $this->db->query($sql);
